...
func ParseTCBVersionV0(values url.Values) (TCBVersionV0, error) {
	...
	for key, valuelist := range values {
		if len(valuelist) != 1 {
			return TCBVersionV0{}, fmt.Errorf("expected exactly one value for KDS TCB version URL argument %q, got %d", key, len(valuelist))
		}
		var target *uint8
		// AMD Publication #57230 Table 13: blSPL, teeSPL, snpSPL are in the range 0..127.
		maxVal := 127
		switch key {
		case "blSPL":
			target = &blSpl
		...
		number, err := strconv.Atoi(valuelist[0])
		if err != nil || number < 0 || number > maxVal {
			return TCBVersionV0{}, fmt.Errorf("invalid KDS TCB version URL argument value %q, want a value 0-%d", valuelist[0], maxVal)
		}
		*target = uint8(number)
	}
	return TCBVersionV0{
...
func (t TCBVersionV1) Values() url.Values {
	values := make(url.Values)
	values.Set("fmcSPL", strconv.Itoa(int(t.FmcSpl)))
	values.Set("blSPL", strconv.Itoa(int(t.BlSpl)))
	values.Set("teeSPL", strconv.Itoa(int(t.TeeSpl)))
	values.Set("snpSPL", strconv.Itoa(int(t.SnpSpl)))
	values.Set("ucodeSPL", strconv.Itoa(int(t.UcodeSpl)))
	return values
}
...
func ParseTCBVersionV1(values url.Values) (TCBVersionV1, error) {
	...
	for key, valuelist := range values {
		if len(valuelist) != 1 {
			return TCBVersionV1{}, fmt.Errorf("expected exactly one value for KDS TCB version URL argument %q, got %d", key, len(valuelist))
		}
		var target *uint8
		...
		number, err := strconv.Atoi(valuelist[0])
		if err != nil || number < 0 || number > maxVal {
			return TCBVersionV1{}, fmt.Errorf("invalid KDS TCB version URL argument value %q, want a value 0-%d", valuelist[0], maxVal)
		}
		*target = uint8(number)
	}
	return TCBVersionV1{
...
func ParseTCBVersion(structVersion uint8, values url.Values) (TCBVersion, error) {
	switch structVersion {
	case 0:
		v0, err := ParseTCBVersionV0(values)
		if err != nil {
			return nil, err
		}
		return v0, nil
	case 1:
		v1, err := ParseTCBVersionV1(values)
		if err != nil {
			return nil, err
		}
		return v1, nil
...
// ParseProductTCB parses query parameters into a TCBVersion based on productLine.
func ParseProductTCB(productLine string, values url.Values) (TCBVersion, error) {
	v, err := StructVersionForProductLine(productLine)
	if err != nil {
		return nil, err
	}
